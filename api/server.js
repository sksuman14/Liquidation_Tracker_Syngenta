// api/server.js   ← SAVE THIS FILE AS:  api/server.js  (in project root)
  ...
  try {
    ...
    const rows = result.rows.map(row => ({
      ...
      record_date: row.record_date 
        ? row.record_date.toLocaleDateString('en-CA')  // YYYY-MM-DD
        : null
    }));
...
app.post("/approve", async (req, res) => {
  ...
  try {
    ...
    const statusFlow = [
      "pending_ta", "approved_by_ta", "approved_by_tsm",
      "approved_by_am", "approved_by_zm", "approved_by_nsm", "fully_approved"
    ];
    ...
    const roleToStatus = {
      TA: "approved_by_ta", TSM: "approved_by_tsm", AM: "approved_by_am",
      ZM: "approved_by_zm", NSM: "approved_by_nsm", CM: "fully_approved"
    };
...
// Remove app.listen() completely – Vercel handles it!