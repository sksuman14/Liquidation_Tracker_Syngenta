// src/pages/ZM.jsx → FINAL PERFECTION WITH + ADD PRODUCT (2025)
...
export default function ZM() {
  ...
  const fetchRequests = async () => {
    try {
      ...
      const zmPending = allRecords.filter(record => record.status === "approved_by_am");
  ...
  return (
    <DashboardLayout title="ZM - Zonal Manager Dashboard">
      <div style={{ padding: 28 }}>
        <h2 style={{ color: "#7c3aed", fontSize: 32, marginBottom: 8 }}>
          Pending ZM Approval ({data.length})
        </h2>
        ...
        {!loading && data.length === 0 && (
          <div style={{
            ...
            No records pending ZM approval
          </div>
        )}
